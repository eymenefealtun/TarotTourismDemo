...
namespace Tourism.DataAccess.Concrete.Models
{
    public class EfOperationMainDal : EfEntityRepositoryBase<OperationMain, AppDbContext>, IOperationMainDal
    {
        public List<OperationMain> GetByDocumentCode(string documentCode)
        {
            ...
            {
                return context.Set<OperationMain>().FromSqlRaw($"SELECT O.DocumentCode,Op.Name'Operator',O.StartDate,O.EndDate, SUM(R.Pax)  'Pax', SUM(R.RoomId)'Room',C.Name 'Currency', O.Id,Sc.Id 'SubCategoryId',SUM(R.DiscountedPrice) 'CurrentIncome' FROM Operations O   FULL JOIN Reservations R ON R.OperationId = O.Id    FULL  JOIN SubCategory Sc ON sc.Id = O.SubCategoryId   FULL  JOIN MainCategory Mc ON Mc.Id = Sc.MainCategoryId    FULL  JOIN Currencies C ON C.Id = O.CurrencyId     FULL  JOIN OperatorUsers Opu ON Opu.Id = O.CreatedBy     FULL  JOIN Operators Op ON Op.Id = Opu.OperatorId   WHERE O.DocumentCode IS NOT NULL AND O.IsActive = 1 AND ( (R.IsActive = 1 OR (R.Pax Is Null OR R.Pax = 0 ) ))  AND O.DocumentCode LIKE '%{documentCode}%'  GROUP BY  O.DocumentCode,O.StartDate,O.EndDate,C.Name,Op.Name,O.Id,Sc.Id  ORDER BY O.StartDate ").AsNoTracking().ToList();
            }
        }
